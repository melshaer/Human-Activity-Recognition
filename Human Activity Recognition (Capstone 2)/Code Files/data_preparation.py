...
data_type = 'train'
save_path = 'gdrive/My Drive/' + data_type + '.json'
...
for imgidx in range(num_images):
  if ((data_type == 'train' and annot_data['img_train'][0][0][0][imgidx] == 1) or (data_type == 'test' and annot_data['img_train'][0][0][0][imgidx] == 0)) and is_empty(annot_data['annolist'][0][0][0][imgidx],'annorect') == False: #any person is annotated 
    #open corresponding image
    img_filename = images_dir_path_str + str(annot_data['annolist'][0][0][0][imgidx]['image'][0][0][0][0]) #image filename
    try:
      ...
      for ridx in range(num_persons):
        if is_empty(annot_data['annolist'][0][0][0][imgidx]['annorect'][0][ridx],'annopoints') == False: #is annotated               
          bbox = np.zeros((4)) #xmin, ymin, width, height
          keypts = np.zeros((num_joints,3)) #x, y, is_visible

          #extract annotated keypoints
          num_annot_joints = len(annot_data['annolist'][0][0][0][imgidx]['annorect'][0][ridx]['annopoints']['point'][0][0][0])
          for jidx in range(num_annot_joints):
            annot_jid = annot_data['annolist'][0][0][0][imgidx]['annorect'][0][ridx]['annopoints']['point'][0][0][0][jidx]['id'][0][0]
            keypts[annot_jid][0] = annot_data['annolist'][0][0][0][imgidx]['annorect'][0][ridx]['annopoints']['point'][0][0][0][jidx]['x'][0][0] #x
            keypts[annot_jid][1] = annot_data['annolist'][0][0][0][imgidx]['annorect'][0][ridx]['annopoints']['point'][0][0][0][jidx]['y'][0][0] #y
        ...
        person_dict = {'person_id': ridx, 'image_id': imgidx, 'area': bbox[2]*bbox[3], 'bbox': bbox.tolist(), 'keypoints_x': annot_keypts[:, 0].tolist(),
                          'keypoints_y': annot_keypts[:, 1].tolist(), 'num_keypoints': int(np.sum(keypts[:,2]==1))}
      ...
      if is_empty(annot_data['act'][0][0][imgidx],'act_name') == False:
        ...
        activity_dict = {'image_id': imgidx, 'activity_name': activity['act_name'].tolist() , 'category_name': activity['cat_name'].tolist(), 'activity_id': activity['act_id'].tolist()}
